...
def lender_detail(cfg: Config, *, option: dict[str, Any], rank: int, district: str | None) -> str | None:
    ...
    prompt = (
        ...
        "Do not claim approval. Do not add phone numbers or legal/financial advice. Keep it concise.\n\n"
...
def selected_lender_conversation(
    ...
    *,
    ...
    prompt = (
        "You are Nudge, a careful WhatsApp chatbot helping an Indian consumer decide whether a local regulated credit option is manageable. "
...
def profile_assessment_message(cfg: Config, *, assessment: str) -> str | None:
    prompt = (
        "Rewrite the credit access profile message below as a warm, empathetic WhatsApp message for an Indian consumer. "
        ...
        "End with a short, encouraging sentence about how Nudge is here to help them find better options. "
        "Keep it natural and under 150 words.\n\n"
        f"Profile message:\n{assessment}"
    )
    reply = generate_reply(cfg, prompt)
    return reply.strip() or None if reply else None
