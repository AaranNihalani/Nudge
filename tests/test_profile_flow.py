...
class TestProfileFlow(unittest.TestCase):
    ...
    def test_invalid_answer_shows_try_again_prefix(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = str(Path(td) / "test.sqlite3")
            init_and_migrate(db_path)
            app = create_app(_make_cfg(db_path))
            ...
            code, reply = chat("-1")
            self.assertEqual(code, 200)
            self.assertIn("please try again", reply.lower())
            self.assertIn("acres", reply.lower())
